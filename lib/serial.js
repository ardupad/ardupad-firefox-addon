...
function setspeed(fd, speed) {
    ...
    ctypesTermios.c_iflag = C_IFLAGS.IGNBRK;
    ctypesTermios.c_oflag = 0;
    ctypesTermios.c_lflag = 0;
    ctypesTermios.c_cflag = (C_CFLAGS.CS8 | C_CFLAGS.CREAD | C_CFLAGS.CLOCAL);
...
function recv(fd, buflen) {
    let ctypesPollfd = new ctypesPollfdStruct;
    let ctypesBuf = new ctypesBufArray;

    let buf = [];
    let len = 0;
    ctypesPollfd.fd.value = fd;
    ctypesPollfd.events.value = POLL.POLLIN;
    while (len < buflen) {
        let nfds = ctypesPoll(ctypesPollfd.address(),
                                       1,
                                       RECV_TIMEOUT);
        if (nfds == 0) {
...
function send(fd, buf) {
    ...
    let len = 0;
    ctypesPollfd.fd.value = fd;
    ctypesPollfd.events.value = POLL.POLLOUT;
    while (len < buflen) {
        let nfds = ctypesPoll(ctypesPollfd.address(),
                                       1,
                                       SEND_TIMEOUT);
        ...
        for (let i = 0;
             i < ((buflen - len > PACKSIZ) ? PACKSIZ : buflen - len);
             i++) {
	    ctypesBuf[i] = buf[i + len];
        }
        let rc = ctypesWrite(fd,
                                      ctypesBuf.address(),
                                      (buflen - len > PACKSIZ) ? PACKSIZ : buflen - len);
        if (rc == -1) {
...
function dtrrts(fd, on) {
    ...
    if (on) {
	ctypesCtl.value |= (MODEM.TIOCM_DTR | MODEM.TIOCM_RTS);
    } else {
	ctypesCtl.value &= ~(MODEM.TIOCM_DTR | MODEM.TIOCM_RTS);
    }
